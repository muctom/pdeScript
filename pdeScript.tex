...
\end{ex}
  ...
  \begin{thm}[Maximum principle for bouneded sets]
    ...
    in \(\Omega_T\), then \[\max_{\bar \Omega_T} u = \max_{\partial^\star \Omega_T} u.\]
  \end{thm}

  \begin{proof}
    We will use Hopf's argument which is simpler that the mean-value theorem (there exists a mean-value theorem for heat equation, buti it is complicated and we will not discuss it). Proof for the Poisson Equation:
    ...
    \begin{enumerate}[label=Step \arabic*)]
      ...
      in \(\Omega \times (0,T]\). Then, beacuse of compactness, there is \((x_0, t_0) \in \bar \Omega \times [0,T]\) s.t. 
      \begin{align*}
        u(x_0, t_0) &= \max_{(x,t) \in \bar \Omega \times [0,T]} u(x,t).
      \end{align*}
      We prove that \((x_0, t_0) \in \partial^\star \Omega_T\). Assume by contradiction that \((x_0, t_0) \notin \partial^\star \Omega_T\), then \(x_0 \in \Omega\) and \(t_0 \in (0,T]\). Since \(x \mapsto u(x,t_0)\) has a (local) maximizer \(x_0 \in \Omega\) we have that \(\Delta_x u(x_0, t_0) \le 0\). Since \(t \mapsto u(x_0, t)\) has a (local) maximizer \(t_0 \in (0,T]\) we have that \(\partial_t u(x_0, t_0) = 0\). This implies:
  ...
  \begin{thm}[Maximum principle for \(\Omega = \mathbb{R}^d\)]
    ...
    \end{itemize}
    Then \[\sup_{\bar \Omega_T} u(x,t) = \sup_{x \in \mathbb{R}^d} u(x,0).\]
  \end{thm}
  ...
  \begin{rem}
    The heat equation 
    \[\partial_t u - \Delta_x u = 0 \\ u(t=0) = g\] is a well-posed problem:
    \begin{itemize}
      \item Existence
      \item Uniqueness
  ...
  \begin{thm}
    If \(u \in C_1^2(\mathbb{R}^d \times [0,T]) \cap C^1(\mathbb{R}^d, [0,T])\) and 
    \[\begin{cases}
      \partial_t u - \Delta_x u = 0 & \text{in } \mathbb{R}^d \times (0,T) \\
      u(x,T) = 0
    \end{cases}\]
  ...
  \section{Instability of the backward heat equation}
  \begin{thm}[Instability]
    There exist functions \(u_\epsilon \)
  \end{thm}
  ...
  \begin{ex}[12.1]
    Let \(u \in C^2(\Omega)\) and let \(x_0 \in \Omega\) be a local minizer. Then 
    \[H = Hu(x_0) = (\partial_i \partial_j u)_{i,j = 1}^d(x_0) \le 0\]
    i.e. \(\xi H \xi \le 0\) for all \(\xi \in \mathbb{R}^d\).
  \end{ex}

  \begin{proof}[Solution]
    In 1D this is obvious if \(x_0\) is a local minimizer of \(u\), then \(u'(x_0) = 0, u''(x_0) \le 0\) (Taylor expansion). \\
    In \(d\) dimensions:
    ...
    \begin{align*}
      \phi''(0) &= \lim_{t \to 0} \frac{\phi'(t) - \phi'(0)}{t} = \lim_{† \to 0} \frac{\nabla u(x_0 + t \xi)}{t} = \lim_{t \to 0}\frac{(\nabla u(x_0 + t \xi) - \nabla u(x_0))\xi}{t} \\
    ...
    \end{align*}
    \(H = (\partial_i \partial_j u(x_0))_{i,j = 1}^d\). Consequently
    \[\Delta u(x_0) = \sum_{i=1}^d \partial_i \partial_j u(x_0) = \operatorname{Tr}(H) \le 0\]
  \end{proof}

  \begin{ex}[E 12.2]
    \[Lu(x) = \sum_{ij} a_{ij}(x) \partial_i \partial_j u(x) + \sum_i b_i(x) \partial_ju(x),\]
      \(a_{ij}, b_i \in C(\bar \Omega)\), \(A(x) = (a_{ij}(x))_{i,j = 1}^d \ge \mathbb{1}\) (as matrics \(A-\mathbb{1} \ge 0\))
      Prove taht if \(Lu(x) \ge 0\) for all \(x \in \Omega\) and \(u \in C^2(\Omega) \cap C(\bar \Omega)\), then \(\max_{x \in \bar \Omega}u(x) = \max_{x \in \partial \Omega} u(x)\)
  \end{ex}

  \begin{proof}[Solution]
    \begin{enumerate}[label=\arabic*)]
      \item \(Lu(x) > 0\) for all \(x \in \Omega\):
      Since \(u \in C(\bar \Omega)\) there is a \(x_0 \in \bar \Omega\) s.t. 
      \[u(x_0) = \max_{x \in \bar \Omega} u(x).\]
      We prove \(x_0 \in \partial \Omega.\) Assume by contradiction that \(x_0 \notin \partial \Omega\), so \(x_0 \in \Omega\) imples that \(x_0\) is a local minimizer. We prove \(Lu(x_0) \le 0\). Note:
      \begin{align*}
        Lu(x_0) &= \sum_{ij} a_{ij}(x) \partial_i \partial_ju(x_0) + \sum_{i} b_i(x_0) \partial_i u(x_0) \\
  ...
  \begin{ex}[E 12.3]
    ...
    \end{cases},\]
    \(f \in C_1^2\) and compactly supported, \(g \in C \cap L^\infty(\mathbb{R}^d)\) and \(u(x,t) \le Me^{M|x|^2}\).
  \end{ex}
  ...
  \end{ex}
  %Hint: Apply half method. But with general operator L. 
