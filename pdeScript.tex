...
\begin{proof}[Proof-Idea]
  Assume there exists \(x_0 \in \Omega\) s.t. \(u(x_0) = \sup_{x \in \bar \Omega} u(x)\). We have that \(B(x_0, r) \subseteq \Omega\), so by the Mean value theorem we have
  \begin{align*}
    u(x_0) 
...
\begin{proof}
  ...
  % \begin{enumerate}
  %   \item[b)] Assume that \(\Omega\) is connected and there is a \(x_0 \in \Omega\) s.t. \(u(x_0) = \sup_{y \in \Omega} u(x)\). Define \(U = \{ x \in \Omega \mid u(x) = u(x_0)\} = u^{-1}(u(x_0))\). \(U\) is closed since \(u\) is continuous. Moreover, \(U\) is open by the mean-value theorem. I.e. for all \(x \in U\) there is a \(r > 0\) s.t. \(B(x,r) \subseteq U \subseteq \Omega\). Since \(U\) is connected we get \(U = \Omega\), so \(u\) is constant in \(\Omega\). On the other hand, if there is no \(x_0 \in \Omega\) s.t. \(u(x_0) = \sup_{x \in \Omega} we have \(\forall x_0 \in \Omega: \quad u(x) < \sup_{x \in \bar \Omega} u(x) = \sup_{x \in \partial \Omega} u(x)\)
  % \end{enumerate}
