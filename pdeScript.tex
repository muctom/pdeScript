...
\begin{document}
  ...
  \begin{proof}[Solution]
    We have already seen that if \(f, g \in W_{loc}^{1,1}(\mathbb{R}^d)\), then \(\max(f,g) \in W_{loc}^{1,1}\). \(u^+, v^+ \in W^{1,1}_{loc}\) for \(v \in W_{loc}^{1,1}\), \(u = u^+ - u^-\) implies \(\nabla u  = \nabla u^+  -\nabla u^-\), \(u^- = \max(-u, 0)\). Claim:
    \begin{align*}
      \nabla u^+ = \begin{cases}
        0 & u(x) \le 0 \\ \nabla u & u(x) > 0
      \end{cases} \quad 
    ...
    \begin{align*}
      ...
      &= - \int_{\mathbb{R}^d} u^+ \partial_i \phi = \int_{\{u(x) \le 0\}} 0 \phi_i \phi - \int_{\{u(x) > 0\}} u \partial_i \phi \\
  ...
  \begin{ex}[E 8.2]
    \(\Omega, U \subseteq \mathbb{R}^d\), open, \(U \cap \Omega \ne \emptyset\), \(u \in W_0^{1,p}(\Omega)\), \(1 \le p < \infty\), \(\chi \in C_c^\infty(U)\). Prove: \(\chi u \in W_0^{1,p}(\Omega \cap U)\)
    Hint: Recall \(W_0^{1,p}(\Omega) = \overline{C_c^\infty(\Omega)}^{\|\cdot\|_{W^{1,p}}}\)
  \end{ex}

  \begin{proof}[Solution]
    % By definition there is a sequence \((u_n)_{n \in \mathbb{N}} \substeq C_c^\infty(\Omega)\) s.t. \(u_n \xrightarrow[n \to \infty]{\|\cdot\|_{W^{1,p}}} u\) , i.e.
    \begin{align*}
    ...
    \begin{align*}
      \|f_n - f_m\|_p = \|\chi(u_nn - u_m)\|_p \le \|\chi\|_\infty \underbrace{\|u_n - u_m\|_p}_{\xrightarrow{n,m \to \infty} 0} \xrightarrow{n,m \to \infty} 0 
    \end{align*}
    \(\nabla f_n = \nabla (\chi u_n) = (\nabla \chi) u_n + \chi \nabla u_n\)
    \begin{align*}
      \|\nabla f_n - \nabla f_m\|_p 
      &\le \|\nabla \chi(u_n - u-m)\|_p + \|\chi(\nabla u_n - \nabla u_m)\|_p \\
      &\le \|\nabla \chi\|_\infty \underbrace{u_n - u_m}_{\xrightarrow{n,m \to \infty} 0} + \underbrace{\|\chi\|}_{< \infty} \underbrace{\|\nabla u_n - \nabla u_m\|_p}_{\xrightarrow{n,m \to \infty}0}\xrightarrow{n,m \to \infty)} 0
    \end{align*}
  ...
  \begin{proof}[Solution]
    ...
    \end{align*}
    Conclusion: Since \((f_n)_{n \in \mathbb{N}} \subseteq C_c^1(U)\) are Cauchy with respect to \(\|\cdot\|_{W^{1,p}}\), there is a \(f \in W_0^{1,p}(U)\) s.t. \(f_n \xrightarrow[\|\cdot\|_{W^{1,p}}]{n \to \infty}f\). Since limits in \(L^p\) are unique by claim 2 we get \(u \circ h = f \in W_0^{1,p}(U)\).
  \end{proof}
