...
\begin{document}
  ...
  \begin{thm}[Basic Properties]
    \begin{enumerate}
      ...
      \item (Formel) \(\widehat{f + g}(k) = \hat f(k) \hat g (k)\) if \(f,g\) are nice enough.
    \end{enumerate}
  \end{thm}
  ...
  \begin{eg}["normal functions" are distributions]
    ...
    \[T(\phi) = \int_{\Omega} f(x) \phi(x) \, dx \quad \forall \phi \in D(\Omega)\]
    is a distribution, i.e. \(T \in D'(\Omega)\). Indeed, it is clear that \(T(\phi)\) is well-defined for all \(\phi \in D(\Omega)\) and \(\phi \mapsto T(\phi)\) is linear. Let us check that \(\phi \mapsto T(\phi)\) is continuous. Take \(\phi_n \to \phi\) in \(D(\Omega)\) and prove that \(T(\phi_n) \to T(\phi)\). Since \(\phi_n \to \phi\) in \(D(\Omega)\), there is a compact \(K\) s.t. \(\supp(\phi_n), \supp(\phi) \subseteq K \subseteq \Omega\).
  \end{eg}
  ...
  \begin{lem}[Fundamental lemma of calculus of variants]
    ...
    \(\int_\Omega f \phi = \int_{\Omega} g \phi\) for all \(\phi \in D(\Omega) \), then \(f = g\) in \(L_{loc}^1(\Omega)\)
  \end{lem}
  ...
  \begin{eg}[Principle Value]
    ...
    \[\int_{\mathbb{R}} f \phi = \int_{\mathbb{R}} \frac{\phi(x)}{x} \, dx\] for all \(\phi \in D(\mathbb{R})\) s.t. \(\phi(0) = 0\). In fact, \(|\phi(x)| = |\phi(x)-\phi(0)| \le (\sup |\phi'|)|x|\), so \(\frac{|\phi(x)|}{|x|} \in L^\infty(\mathbb{R})\) and compactly supportet. So \(\frac{\phi(x)}{x} \in L^1(\mathbb{R})\).
    ...
    \end{align*}
    We denote \(T = p.v. \left(\frac{1}{x}\right)\). Check that \(T \in D'(\mathbb{R}: \) For all \(\epsilon > 0, \) \(\left|\frac{\phi(x)}{x} \right| \le \frac{\|\phi\|_{L^\infty}}{\epsilon}\) for all \(|x| \ge \epsilon\) and \(\phi\) is compactly supported. So we get for all \(\epsilon > 0\):
    \begin{align*}
      \mathbb{1}(|x| \ge \epsilon)\frac{\phi(x)}{x} \in L^1(\mathbb{R}) \leadsto \int_{|x| \ge \epsilon} \frac{\phi(x)}{x} \, dx \text{ finite.}
    \end{align*}
    We can write:
    \begin{align*}
      \int_{|x| \ge \epsilon} \frac{\phi(x)}{x} \, dx = \int_{|x| \ge 1} \frac{\phi(x)}{x} \, dx + \int_{\epsilon \le |x| \le 1} \frac{\phi(x)}{x}\, dx
    \end{align*}
    The second part can be written as:
    \begin{align*}
      \int_{\epsilon \le |x| \le 1} \frac{\phi(x)}{x} \, dx
      &= \int_{\epsilon} \frac{\phi(x)}{x} \, dx + \int_{-1}^{-\epsilon} \frac{\phi(x)}{x} \, dx 
      = \int_\epsilon^1 \frac{\phi(x) - \phi(-x)}{x}\, dx
    \end{align*}
    Since \(\phi \in C_c^\infty(\mathbb{R})\)
    \begin{align*}
      |\phi(x) - \phi(-x)| \le 2 \|\phi'\| \le 2 \|\phi'\|_{L^\infty} |x|
    \end{align*}
    \begin{align*}
  ...
  \begin{defn}[Derivatives of distributions] 
    Let \(\Omega \subseteq \mathbb{R}^d\) and \(T \in D'(\Omega)\). Define 
    \begin{align*}
      D^\alpha T: \ D(\Omega) &\to \mathbb{R} \text{ or } \mathbb{C} \\
      (D^\alpha T)(\phi)= (-1)^{|\alpha|}T(\underbrace{D^\alpha \phi}_{in D(\Omega)}), \quad |\alpha| = \alpha_1 + \dots + \alpha_d \text{ with } \alpha = (\alpha_1, \dots, \alpha_d)
    \end{align*}
  ...
  \begin{align*}
    ...
    (D^\alpha T)(\phi) \leftarrow T_{D^\alpha f} (\phi) = (-1)^{|\alpha|}T_f(d^\alpha \phi)
  \end{align*}
  "If the classical derivatives exist, then They are the same with the distributional derivatives"

  
  \begin{rem}
    For all \(T \in D'(\Omega)\), then \(D^\alpha T \in D'(\Omega)\) for all \(\alpha = (\alpha_1, \dots, \alpha_d)\). Clearly:
    \[\phi \mapsto (D^\alpha T)(\phi) = (-1)^{|\alpha|}T(D^\alpha \phi)\] linear. Moreover, if \(\phi_n \to \phi\) in \(D(\Omega)\): 
    \begin{align*}
      \Rightarrow D^\alpha \phi_n \to D^\alpha \phi \text{ in } D(\Omega) \\
      \Rightarrow (D^\alpha T)(\phi_n) = (-1)^\alpha T(D^\alpha \phi_n) \xrightarrow{n \to \infty} (-1)^\alpha T(D^\alpha \phi) = (D^\alpha T)(\phi)
    \end{align*}
  \end{rem}

  \begin{eg}
    Consider \(f(x) = |x|\) in \(\mathbb{R}\). Then \(f \in C(\mathbb{R})\) but \(f \notin C^1(\mathbb{R})\). However, 
    \[f'(x) = g(x) = \begin{cases}
      1 & x > 0 \\ -1 &x > 0
    ...
    namely:
    \begin{align*}
      - \int_{\mathbb{R}} |x| \phi'(x) \, dx &= \int_0^\infty \phi(x) \, dx - \int_{-\infty}^0 \phi(x) \, dx
    \end{align*}
    \begin{align*}
      LHS = - \int_0^\infty x \phi'(x) \, dx + \int_{-\infty}^{\infty} x \phi'(x) \, dx
    \end{align*}
    ...
    Claim: \(g' = 2 \delta_0\) in \(D'(\mathbb{R})\). In fact, for all \(\phi \in D(\mathbb{R})\), then:
    \begin{align*}
      g'(\phi) 
      &= - g(\phi') 
      = - \int_{\mathbb{R}} g\phi' 
      = - \int_{-\infty}^0 (-1) \phi' - \int_0^\infty (1) \phi' \\
      &= - \int_0^\infty \phi' + \int_{-\infty}^0 \phi' = [\phi(0) - \underbrace{\phi(\infty)}_{= 0}] + [\phi(0) - \underbrace{\phi(-\infty)}_{= 0}]
  ...
  \begin{ex}
    Prove that \((D^\alpha \delta_x)(\phi) = (-1)^{|\alpha|}(\Delta^\alpha \phi)(x)\) for all \(\phi \in D(\mathbb{R})\) for all \(x \in \mathbb{R}\).
  \end{ex}

  \begin{defn}[Convergence of distributions]
    Let \(\Omega \subseteq \mathbb{R}^d\) open, then
  ...
  \begin{thm}
    For all \(T \in D'(\mathbb{R})\), for all \(f \in D(\mathbb{R})\), then
    \begin{enumerate}
  ...
  \begin{proof}
    \begin{enumerate}
      \item We prove that \(y \mapsto T(f_y)\) is continous. Take \(y_n \to y\) in \(\mathbb{R}^d\), then:
      \item \[T(f_{y_n}) \to T(f_y)\] since \(f_{y_n} \to f_y\) in \(D(\mathbb{R}^d)\) \((\star)\)
    \end{enumerate}
    Check \((\star)\): Since \(f \subseteq C_c^\infty(\mathbb{R}^d)\Rightarrow \supp f \subseteq B(0, R) \subseteq \mathbb{R}^d\). Since \(y_n \to y\) in \(\mathbb{R}^d\). We have \(\sup_n |y_n| < \infty\). Thus \(f_{y_n}, f_y\) are supported in \(\overline{B(0, R + \sup_n |y_n|)} = K\) compact. Moreover
    \begin{align*}
      |f_{y_n}(x) - f_y(x)|
      &= |f(x-y_n) - f(x-y))|
      \le \| \delta f \|_{L^\infty} \|y_n - y\| \to 0
    \end{align*}
    So we get \(\| f_{y_n} - f_y\|_{L^\infty} \to 0\)
    Similary:
    \[\|D^\alpha f_{y_n} - D^\alpha f_n \|_{L^\infty} \ to 0\]
    Thus \(f_{y_n} \to f_y\) in \(D(\mathbb{R}^d)\) i.e. \((\star)\) holds.
  \end{proof}
  ...
  \begin{ex}[E 3.1 Lebesgue Differentiation Theorem]
    ...
    \[\fint_{B(x,r)} |f(y)  f(x)| \, dy \xrightarrow{r \to 0} 0\]
  \end{ex}

  \begin{proof}[Solution]
    If \(g \in L^(\mathbb{R}^d) \int g = 1, g_\epsilon(x) = \epsilon^{-d} g(\epsilon^{-1} x)\), then: \(g_\epsilon \star f \to f\) in \(L^1(\mathbb{R}^d)\)
    
    \begin{lem}
      If \(f_n \to f\) in \(L^1(\Omega)\), then there is a subsequence \(\{f_{n_k}\}\) s.t. we have the Dominated Convergence 
      \begin{align*}
        \begin{cases}
          f_{n_k} (x) \to f(x) a.e. \\
          |(f_{n_k}(x))| \le F(x) \in L^1(\Omega)
        \end{cases}
      \end{align*}
    \end{lem}
      
      Consequence of the proof of the completeness of \(L^1(\Omega)\). Consequence:
      \begin{align*}
        \int_{B(x, r)} f 
        &= \int_{\mathbb{R}^d} \frac{\mathbb{1}(B(x, r))(y)}{|B(x,r)|} \, dy 
        = \int_{\mathbb{R}^d} \frac{\mathbb{1}(B(0,r))}{|B(0,r)} (x-y) f(y) \, dy \\
        &= (g_r \star f) (x) , \quad g_r = \frac{\mathbb{1}(B(0,r))}{|B(0,r)|} \text{(similar to \(g_\epsilon\))}
      \end{align*}
      We le that if \(f \in L^1\), then: \(g_r \star f \xrightarrow{r \to 0} f\) in \(L^1\). So up to a subsequence we have \((g_r \star f)(x) \to f(x)\) a.e.
      Here the Lebesgue differentation then is stronger:
      \begin{itemize}
        \item \(g_r \star f(x) \to f(x)\) as \(r \to 0\) a.e. x. 
        \item \begin{align*}
          \left| (g_r \star f)(x) - f(x) \right|
          &= \left| \fint_{B(x,r)} (f(y) - f(x)) \, dy \right| 
          = \fint_{B(x,r)} |f(y) - f(x)| \, dy
        \end{align*}
      \end{itemize}
      \begin{enumerate}[label=Step \arabic*)]
        \item Assume \(f \in C(\mathbb{R}^d)\). Then:
        \[\fint_{B(x,r)} |f(y) - f(x)| \, dy \le \sup_{|y-x| \le r} |f(y) - f(x)| \to 0 \quad \forall x \in \mathbb{R}^d\] 
        since \(f\) is continuous
        \item Take \(f \in L^1(\mathbb{R}^d)\), then \(\exists \{f_n \subseteq C_c(\mathbb{R}^d)\}\) s.t. \(f_n \to f\) in \(L^1(\mathbb{R}^d)\) and \(f_n(x) \to f(x)\) for a.e. \(x \in \mathbb{R}^d\).
        \begin{align*}
          &\fint_{B(x,r)} |f(y) - f(x)| \, dy \\
          &= \fint_{B(x,r)} \underbrace{|f(y) - f_n(y)}_{|f(x) - f_n(x)} + \int_{B(x, r)}|f_n(y) - f_n(x)| \, dy + \fint_{B(x,r)} |f_n(x) - f(x)| \, dy \\
          \Rightarrow &\limsup_{n \to 0} \fint_{B(x,r)} |f(y) - f(x)| \, dy \le 2 |f(x) - f_n(x)| \xrightarrow{n \to \infty} 0
        \end{align*}
        Claim without proof: For all \(g \in L^1\)
        \[\fint_{B(x,r)} g(y) \, dy \xrightarrow{r \to 0} g(x)\]
       \end{enumerate}
  \end{proof}


  \begin{ex}[E 3.2]
    Let \(1 \le p, q, r \le 2\), \(\frac{1}{p} + \frac{1}{q} = 1 + \frac{1}{r}\). Take \(f \in L^p, g \in L^q\), prove:
  ...
  \begin{proof}[Solution]
    \begin{enumerate}[label=Step \arabic*)]
      ...
      \begin{align*}
        ...
        \text{(Hausdorff-Young)} \quad &\le \|f \star g\|_{L^r} 
        = \|(f - f_n) \star g_n + f_n \star (g_n - g) \|_{L^r} \\
        &\le \| (f-f_n) \star g_n \|_{L^r} + \| f_n \star (g_n - g)\|_{L^r} \\
        \text{(Young)} &\le \|f - f_n\|_{L^p} \|g_n\| + \|f_n\|_{L^p} \|g_n - g\|_{L^p} \xrightarrow{n \to \infty}
      \end{align*}
      Moreover:
      \begin{align*}
        ...
        \text{(Hausdorff-Young)} \quad &\le \| f_n - f \|_{L^p}\|g_n\|_{L^q} + \|f\|_{L^p} \|g_n - g\|_{L^p} \xrightarrow{n \to \infty} 0
      \end{align*}
  ...
  \begin{proof}
    \begin{enumerate}[label=\alph*)]
      ...
      \begin{itemize}
        ...
        \begin{align*}
          ...
          = \frac{f(x-y+he_i) - f(x-y)}{h} - (\partial_i f)(x-y) \\
      ...
      By induction: 
      \begin{align*}
        D_y^\alpha T(f_y) = (-1)^{|\alpha|} T((D^\alpha f)_y) = (D^\alpha T)(f_y) \quad \forall \alpha
      \end{align*}
    \end{enumerate}
    ...
    \begin{enumerate}[label=Step \arabic*:]
      ...
      Finally we optain:
      \[\int g(y) T(f_n) \, dy = T(g \star f)\]
    \end{enumerate}
  \end{proof}

  
  \begin{thm}
    Let \(\Omega \subseteq \mathbb{R}^d\) be open. Let \(T \i D'(\Omega)\) and \(f \in C_c^\infty(\Omega)\). Denote
    ...
    \begin{enumerate}[label=\alph*)]
      ...
      \item For all \(g \in L^1(\Omega_g)\) and compactly supported in \(\Omega_f\) , then:
  ...
  \begin{rem}
    If \(u \in C^1(\mathbb{R}^d)\), then:
      \begin{align*}
        u(x+y) - u(x) 
        &= \int_0^1 \sum_{j=1}^d y_j u(x + ty_j) \, dt \\
        &= \int_0^1 y \nabla u(x+ty) \, dt
      \end{align*}
  ...
  \begin{thm}[Taylor expansion for distributions]
    Let \(T \in D'(\mathbb{R}^d)\) and \(f \in C_c^\infty(\mathbb{R}^d)\). Then: \[T(f_y) - T(f) = \int_0^1 \sum_{j=1}^d y_j(\partial_j T) (f_{ty}) \, dt\]
  \end{thm}
  ...
  \begin{proof}
    ...
    \begin{align*}
      ...
      &= \int_0^1 (\nabla T)(f_{ty}) y \, dt \\
      &= \int_0^1 \sum_{j=1}^d (\partial_j T)(f_{ty}) y_j \, dt
    \end{align*}
  \end{proof}
